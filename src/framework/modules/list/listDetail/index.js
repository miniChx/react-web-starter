...
class ListDetail extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      passwordDirty: false,
      model: getValueByKey(this.props, 'show', 'location', 'query', 'model')
    };
  }
  ...
  handleChageState(e) {
    dispatch(replace({
      pathname: '/' + CONTAINER_PRE + '/' + this.props.domainLink,
      query: { ...this.props.params, domainType: this.props.domainType, model: (this.state.model === 'show' ? 'edit' : 'show') }
    }));
  }
  ...
  renderForm(data, index) {
    ...
    return (
      <div>
        <h2><AnHref title="基本信息" href={'#components-anchor-demo-basic' + index} /></h2>
  ...
  render() {
    ...
    return (
      <div>
        {this.renderForm(this.props.dataSource, 1)}
        {this.renderForm(this.props.dataSource, 2)}
        {this.renderForm(this.props.dataSource, 3)}
        <Form horizontal={true} >
