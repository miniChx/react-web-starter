...
class App extends Component {
  ...
  _resolvePath(path, resolveRoutes) {
    const route = this.props.routes.find(routeItem => routeItem.path === path);
  ...
  _createBreadcrumbItems(path) {
    ...
    const breadcrumbItems = resolveRoutes.map((resolveRoute, index, routes) => {
  ...
  _replacePath(path, url) {
    ...
    const newPath = pathArr.map((pathStr, index) => {
      return urlArr[index];
    }).join('/');
    return newPath;
  }
  ...
  _createBreadcrumbItem(route, index, routes) {
    ...
    if ((/:/).test(route.path)) {
      const path = this._replacePath(route.path, this.props.match.url);
      return (
        <Breadcrumb.Item key={route.path} className={'breadcrumb-placeholder'}><Link to={path}>{breadcrumbName}</Link></Breadcrumb.Item>
      );
    }
    return (
